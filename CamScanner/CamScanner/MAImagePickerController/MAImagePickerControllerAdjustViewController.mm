...
//    largest_square = largestRect(original);
//    findFile(original, squares);
...
cv::vector<cv::Point> largestRect(cv::Mat& image)
{
    cv::Mat hsv;
    cvtColor(image, hsv, CV_BGR2HSV);
    cv::vector<cv::Mat> channels;
    split(hsv, channels);
    cv::vector<cv::Point> rect;
    double maxArea = 0.0;
    for (int i = 0; i <= 90; i += 1)
    {
        ...
        for (int i = 0; i < contours.size(); ++i)
        {
            ...
            cv::approxPolyDP(contours[i], polygon, cv::arcLength(contours[i], 1) * 0.02, 1);
...
void findFile(cv::Mat & image,cv::vector<cv::vector<cv::Point>>&squares){
    IplImage img = image;
    ...
    cv::Mat inputImg = cv::Mat(cvMat);
    
    // blur will enhance edge detection
    cv::Mat blurred(image);
    
//    medianBlur(image, blurred, 9);
    cv::pyrMeanShiftFiltering(inputImg,blurred,25, 10);
...
void find_squares(cv::Mat& image, cv::vector<cv::vector<cv::Point>>&squares) {
    ...
    // blur will enhance edge detection
    cv::Mat blurred(image);
   // GaussianBlur(image, blurred, cv::Size(3, 3), 0, 0);
    medianBlur(image, blurred, 9);
